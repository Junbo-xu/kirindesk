...
// Phase 1F-C: sales order approval workflow. Boots the real Nest app with the
// same global ValidationPipe as src/main.ts. Role matrix (from fixtures):
//   admin  (TEST_USER)  -> orders:* at scope ALL  (can approve any order)
//   sales  (TEST_USER2) -> orders:* at scope OWN   (approve rejected: not all-scope)
//   noperm (TEST_USER4) -> no roles                (401/403 gate)
//   t2admin(TEST_USER3) -> tenant2 admin           (cross-tenant isolation)
describe('Sales Orders Approval (integration)', () => {
  ...
  // Creates a draft order owned by the caller and returns its id.
  ...
  beforeAll(async () => {
    ...
    adminCustomerId = await createCustomer(adminToken, 'Approval Admin Customer');
    salesCustomerId = await createCustomer(salesToken, 'Approval Sales Customer');
  });
  ...
  it('submit with no token -> 401', async () => {
    const id = await createDraft(adminToken, adminCustomerId, 'SO-AP-AUTH');
    const res = await request(app.getHttpServer()).post(`/api/sales-orders/${id}/submit`);
    expect(res.status).toBe(401);
  });

  it('submit by a user lacking orders:update -> 403', async () => {
    const id = await createDraft(adminToken, adminCustomerId, 'SO-AP-NOPERM');
    const res = await request(app.getHttpServer())
  ...
  it('sales submits own draft -> 200, status pending_approval', async () => {
    const id = await createDraft(salesToken, salesCustomerId, 'SO-AP-HAPPY');
    const res = await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/submit`)
  ...
  it('admin (all-scope) approves the sales-submitted order -> 200, status approved', async () => {
    const res = await request(app.getHttpServer())
      .post(`/api/sales-orders/${happyOrderId}/approve`)
  ...
  // --- reject path (admin rejects a sales submission) ---

  it('reject without a reason -> 400', async () => {
    const id = await createDraft(salesToken, salesCustomerId, 'SO-AP-REJ-NOREASON');
  ...
  it('admin rejects a pending order with a reason -> 200, status rejected', async () => {
    const id = await createDraft(salesToken, salesCustomerId, 'SO-AP-REJECT');
    await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/submit`)
      .set(bearer(salesToken))
      .expect(200);
  ...
  // --- withdraw path (submitter recalls before a decision) ---

  it('sales withdraws its own pending order -> 200, back to draft', async () => {
    const id = await createDraft(salesToken, salesCustomerId, 'SO-AP-WITHDRAW');
    await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/submit`)
      .set(bearer(salesToken))
      .expect(200);
  ...
  it('admin approving its own submission -> 403 (self-approval blocked)', async () => {
    const id = await createDraft(adminToken, adminCustomerId, 'SO-AP-SELF');
    await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/submit`)
      .set(bearer(adminToken))
      .expect(200);
  ...
  // --- all-scope requirement: own-scoped approver is rejected (scope check
  // fires before the separation-of-duties check) ---

  it('sales (own-scope) approving -> 403 (approval requires all-scope)', async () => {
    const id = await createDraft(salesToken, salesCustomerId, 'SO-AP-OWNSCOPE');
    await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/submit`)
      .set(bearer(salesToken))
      .expect(200);
  ...
  it('approve a draft order -> 409', async () => {
    const id = await createDraft(adminToken, adminCustomerId, 'SO-AP-ILLEGAL-APPROVE');
    const res = await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/approve`)
      .set(bearer(adminToken))
      .send({});
  ...
  it('submit an already-pending order -> 409', async () => {
    const id = await createDraft(salesToken, salesCustomerId, 'SO-AP-DOUBLE-SUBMIT');
    await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/submit`)
      .set(bearer(salesToken))
      .expect(200);
  ...
  it('withdraw an approved order -> 409 (approved is forward-only)', async () => {
    const id = await createDraft(salesToken, salesCustomerId, 'SO-AP-WD-APPROVED');
    await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/submit`)
      .set(bearer(salesToken))
      .expect(200);
  ...
  // --- 404: not found / out of scope / cross-tenant ---

  it('transition on a non-existent order -> 404', async () => {
    const res = await request(app.getHttpServer())
      .post('/api/sales-orders/00000000-0000-0000-0000-000000000000/submit')
      .set(bearer(adminToken));
    expect(res.status).toBe(404);
  });

  it('tenant2 admin cannot submit a tenant1 order -> 404', async () => {
    const id = await createDraft(adminToken, adminCustomerId, 'SO-AP-XTENANT');
    const res = await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/submit`)
  ...
  it('order_approvals ledger records each transition with the actor', async () => {
    const id = await createDraft(salesToken, salesCustomerId, 'SO-AP-LEDGER');
    await request(app.getHttpServer())
      .post(`/api/sales-orders/${id}/submit`)
      .set(bearer(salesToken))
      .expect(200);
    ...
    const rows = await withAdmin(async (c) => {
      const r = await c.query(
        `SELECT action, from_status, to_status, actor_user_id, reason
          ...
          WHERE order_type = 'sales' AND order_id = $1
  ...
  it('approval audit actions are recorded for tenant1', async () => {
    ...
    expect(actions).toContain('sales_order.submitted');
    expect(actions).toContain('sales_order.approved');
    expect(actions).toContain('sales_order.rejected');
    expect(actions).toContain('sales_order.withdrawn');
  });
