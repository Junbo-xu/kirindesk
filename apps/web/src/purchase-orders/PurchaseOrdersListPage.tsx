...
import { ApiError, OrderStatus, PurchaseOrderResponse } from '../lib/types';
...
export function PurchaseOrdersListPage() {
  ...
  const supplierName = useMemo(() => {
    const map = new Map(suppliers.map((s) => [s.id, s.company_name]));
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .catch((err) => {
        if (!active) return;
        setError(err instanceof ApiError ? err.message : '加载采购订单失败，请稍后重试');
      })
      .finally(() => {
  ...
  async function onDelete(order: PurchaseOrderResponse) {
    if (!window.confirm(`确认删除采购订单 ${order.order_number}？此操作不可撤销。`)) return;
    setError(null);
    try {
      ...
      if (status === 404) setError('采购订单不存在或已被删除');
  ...
  async function reload() {
    setLoading(true);
    try {
      const res = await apiClient.listPurchaseOrders({
        page,
        pageSize: PAGE_SIZE,
        q: appliedQ || undefined,
        status: appliedStatus || undefined,
  ...
  return (
    <div style={{ fontFamily: 'system-ui' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h1 style={{ fontSize: 20 }}>采购订单</h1>
        <Link to="/purchase-orders/new">新建采购订单</Link>
      </div>
          ...
          <tbody>
            {orders.map((o) => (
              <tr key={o.id}>
                ...
                <td style={td}>{supplierName(o.supplier_id)}</td>
                ...
                <td style={td}>
                  <Link to={`/purchase-orders/${o.id}/edit`}>编辑</Link>{' '}
