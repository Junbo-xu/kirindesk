    ...
    <div style={{ maxWidth: 720, fontFamily: 'system-ui' }}>
      ...
      {isEdit && id && (
        <OrderApprovalActions
          status={status as OrderStatus}
          handlers={{
            submit: () => apiClient.submitPurchaseOrder(id),
            approve: (reason) => apiClient.approvePurchaseOrder(id, reason),
            reject: (reason) => apiClient.rejectPurchaseOrder(id, reason),
            withdraw: (reason) => apiClient.withdrawPurchaseOrder(id, reason),
          }}
