...
当前基线已包含阶段 2B–2F。PI、收款、采购申请/采购单、到货/QC、发货、费用、财务核对、利润、提成修订、样品和售后领域服务均在各自事务中追加最小业务事件。时间线查询会在租户内解析询盘、销售订单、关联采购订单和样品转订单关系，因此从任一已知链根筛选时可聚合前后凭证，同时仍按每条事件的底层资源权限和数据范围过滤。
...
- 指定链根时，服务端使用租户内关系图解析 `inquiry`、`sales_order`、`purchase_order` 及样品转订单连接；关系解析只扩大链根集合，不扩大事件可见权限。
...
- 2G 专项集成：7 个用例通过，覆盖五类角色工作台、无权限 403、跨租户 RLS、四类异常、状态机/CAS、assigned 时间线、询盘/销售订单双向全链解析、跨链权限反向断言、供应商证据反向断言、事件不可变和审计链完整性。
...
- 新增业务域如果引入新的链根类型，必须同时在领域事务中追加事件，并在关系图中显式登记与询盘、销售订单或采购订单的连接；未知类型保持只匹配自身，避免默认扩张。
- 当前关系解析由 RLS 和显式租户条件双重约束。后续修改关系图时必须保留跨租户反向测试与底层权限反向测试。
